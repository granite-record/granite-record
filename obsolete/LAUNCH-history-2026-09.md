# `LAUNCH.md` as it stood, 9-17 September 2026

Split out of `LAUNCH.md` on 17 September, before the repository goes public.

For nine days `LAUNCH.md` was a running log as well as a list: every decision
with the measurement behind it, every bug with the reason it happened, what the
person asked for and on which evening. That is the part of this project most
worth keeping and the part a stranger cannot read past. So the log is here and
the list is there.

**`LAUNCH.md` is the live one.** Nothing below is a statement about the site
today. Every count, every "running now", every "open" in this file was true on
the day it was typed, and several were wrong by the next morning -- which is
the whole reason `STATE.md` is generated. Where this file and `LAUNCH.md`
disagree, `LAUNCH.md` wins; where either disagrees with `STATE.md`, `STATE.md`
wins. The commit hashes quoted here were remapped onto the rewritten history on
17 September and resolve in this repository.

This is the file entire, not an extract of the parts that came out, so a few
paragraphs appear in both places. That is cheaper than a reader wondering
whether a sentence was cut or moved, and it is the convention the rest of
`obsolete/` already follows: what is superseded is kept whole.
